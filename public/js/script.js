...
navigator.mediaDevices.getUserMedia({
    ...
    socket.on("user-connected", (data) =>{      //on when a user connects to the room
    ...
    //Chat functionality
    $("html").keydown(function (e){     //when user presses enter
        if (e.which == 13 ){ 
            sendMessage();
        }
    });
...
socket.on("user-disconnected", (data) =>{
    $(".messages").append(`<li class="disconnect"><i>${data.username} has left the meeting.</i></li>`);
    $(`.${data.userId}`).remove();
...
myPeer.on("open", userId =>{      //when connection is opened by a specific (user) id
...
const connectToUser= (userId, stream, username) =>{     //calls this function for others when a new user connects to the room
...
const sendMessage= () =>{
    ...
    if (txt.length !== 0){
        ...
        $(".messages").append(`<li class="msg my-text"><b>You</b><br/><span>${txt}</span></li>`);
    }
    text.val("");
}
...
socket.on("all-users", (memberNames, memberIds) =>{
    for(let i=0; i< memberNames.length; i++){
        if(memberIds[i]!==myUserId){
            $(".people").append(`<li class="ppl ${memberIds[i]}">${memberNames[i]}<hr></li>`);   //append name to member list
        }     
    }       
})
